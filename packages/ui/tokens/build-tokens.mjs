...
const primitive = {
  ...
  shadow: {
    soft: { $type: 'shadow', $value: '0 1px 2px rgba(120,90,70,0.03), 0 4px 12px -6px rgba(120,90,70,0.10)' },
    pop: { $type: 'shadow', $value: '0 4px 12px -2px rgba(120,90,70,0.06), 0 12px 28px -8px rgba(120,90,70,0.14)' },
  },
}
...
// Emit 5: primitive ramps as CSS custom properties + Tailwind @theme registration,
// so `bg-brand-600` / `text-error-700` become real utilities. Imported by theme.css.
const cssVars = ramps
  .flatMap((r) => r.steps.map((s) => `  --${r.name}-${s.step}: ${s.oklch};`))
  .join('\n')
const cssTheme = ramps
  .flatMap((r) => r.steps.map((s) => `  --color-${r.name}-${s.step}: var(--${r.name}-${s.step});`))
  .join('\n')
const css = `/* AUTO-GENERATED by tokens/build-tokens.mjs — do not edit by hand. */
/* Primitive OKLCH ramps. Ramps are theme-independent; only semantics flip in .dark. */
:root {
${cssVars}
}
