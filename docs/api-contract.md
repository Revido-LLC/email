...
**Status:** planning doc for the backend build. Not yet implemented — `apps/api` is a Wave 0
health-check stub (`apps/api/src/index.ts`).

## Purpose

Today the product is a UI-only React SPA rendered over `@revido/mock-data`: a typed fake mailbox
that was deliberately shaped like the future API (`packages/mock-data/src/types.ts` mirrors
`packages/db/src/domain.ts` field-for-field). This document turns that mock into a real endpoint
surface:

- **The mock's 14 getter functions** (`packages/mock-data/src/index.ts`) become 14 read endpoints,
  one-to-one.
- **Every place the UI mutates local React state** to simulate an action — archiving, sending,
  approving, toggling an agent, saving a signature — becomes a write endpoint.
- **The plan's AI, agent, OAuth, webhook, and lead surfaces**, which have no mock stand-in at all
  because they don't fit a "read fake data" model, are specified from the product plan
  (`docs/information-architecture.md`) and the reserved shapes already stubbed in
...
| *(none — static)* | *n/a* | `CategoryMeta[]` | The 9 locked categories (`packages/mock-data/src/categories.ts` `CATEGORIES`/`CATEGORY_LIST`) are fixed product taxonomy, not per-user data — ship as a frontend constant in `@revido/db` or `@revido/ui`, not a live endpoint. |
...
| *(none — hardcoded `AI_PREFS` defaults in `app.settings.tsx`)* | `GET /settings/ai` | `{ drafts: boolean; agents: boolean; chat: boolean; digest: boolean }` | `routes/app.settings.tsx` (AI tab initial toggle state) |
