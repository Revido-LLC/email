---
name: mock-data
description: Use when you add mock data / change the data shape / add a field to Thread (or any `@revido/mock-data` type) — the package is the data contract the UI renders against in dev (it mirrors `@revido/db`), so edit it contract-first.
---

# Mock data

`packages/mock-data` is a typed fake mailbox the UI renders against in dev, shaped like the
real API — its types mirror `@revido/db`'s domain types. Screens treat it as the data contract,
so changes here are contract changes — make them deliberately.

## Contract-first

- `src/types.ts` is the contract the UI renders against, mirroring `@revido/db`'s domain
  types. **Edit types before `src/data.ts`.** The data file just conforms to the interfaces.
...
- Expose data through a getter in `src/index.ts` that mirrors an API endpoint. Follow the
