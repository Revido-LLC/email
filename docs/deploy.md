...
| `Postgres`       | `ghcr.io/railwayapp-templates/postgres-ssl:18` | —                          | Railway Postgres 18 (pgvector 0.8.5 + pgcrypto). **Already provisioned + migrated (0000–0008) + RLS-proven.** |
...
Schema is raw SQL under `packages/db/drizzle/` (`0000`–`0008`), split on `--> statement-breakpoint`.
...
(`agent_actions.params`, `agents.trusted`, `approvals.params`/`message_id`); `0007` adds workspaces,
calendar, meetings/transcripts, custom labels, unified content embeddings, artifacts, and billing
webhook state; `0008` normalizes mailbox addresses and enforces one connection per user and email.

**Status: all nine are already applied to the live staging + production DBs, and GUC-RLS is runtime-proven.** To
...
- `S3StorageProvider` is backed by AWS SDK v3 and supports AWS S3 or Cloudflare R2. Meeting recording
  chunks fall back to per-user encrypted Postgres ciphertext when S3/R2 is not configured.
